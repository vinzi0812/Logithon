...
def get_airports_along_route(source_city, destination_city):
    prompt = f"Find all possible airports and regions that a flight would pass over while traveling from {source_city} to {destination_city}. Include airports for direct flights as well as for flights with stopovers. Provide the response in a JSON format with the following structure: [{{'airport': 'airport_name'}}]."
    convo = model.start_chat(history=[])
    convo.send_message(prompt)
    airports_json = convo.last.text
    airports_json = airports_json.replace("```", "").replace("json", "").strip()
    try:
        airports = json.loads(airports_json)
    except json.decoder.JSONDecodeError:
        print(f"Error decoding JSON: {airports_json}")
        airports = []
    return airports
...
def main():
    ...
    safe_areas = [area for area in airports if area["airport"] not in unsafe_areas]
